...
**Lancé en dev :**
- `pytest tests/test_download.py -v` → **85/85 verts**.
- Suite complète → **343/343 verts** (non-régression TICKET-01 à 09 confirmée).
- `whisper_type.py` relu : `model_in_cache()` cross-platform (HF_HOME/HF_HUB_CACHE), `download_model_with_progress()` filtre msgpack/h5, check proactif dans `sidecar_loop()`, commandes `download_model`/`check_model` propres.
- `lib.rs` relu : `handle_download_events()` correctement branché dans `spawn_stdout_reader`, auto-download sur `model_missing`, hide sur `model_ready`/`model_cached`, `retry_download` dans invoke_handler.
- `download.html`/`download.js` relus : animation indéterminée, transitions propres, retry via `core.invoke`, try-catch JSON.
- Téléchargement réseau réel et thread timing : déférés (accès internet + sidecar requis).
- Fix TICKET-04 rétrocompatibilité : `test_sidecar_ipc.py` mis à jour, 17 tests toujours verts.

**Lancé en prod :** N/A.

**DoD complète :** Oui — 5/5 cases.
- Fonctionnement réseau réel : déféré, mockage exhaustif (list_repo_files + hf_hub_download).
